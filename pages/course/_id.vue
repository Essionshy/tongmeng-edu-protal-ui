    ...
    <div class="mt50 commentHtml">
      <div>
        ...
        <div class="paging">
          ...
          <a :class="{undisable: !hasPrevious}" href="#" title="首页" @click.prevent="gotoPage(1)">首</a>
          <a :class="{undisable: !hasPrevious}" href="#" title="前一页" @click.prevent="gotoPage(this.current-1)">&lt;</a>
          <a v-for="page in pages" :key="page" :class="{current: current == page, undisable: current == current}" :title="'第'+current+'页'"
            href="#" @click.prevent="gotoPage(page)">{{ page }}</a>
          <a :class="{undisable: !hasNext}" href="#" title="后一页" @click.prevent="gotoPage(current+1)">&gt;</a>
          <a :class="{undisable: !hasNext}" href="#" title="末页" @click.prevent="gotoPage(pages)">末</a>
          <div class="clear" />
        </div>
        <!-- 公共分页 结束 -->
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //初始化评论列表
      initCommentList() {
        // this.comment.teacherId=this.teacher.id; 首次加载课程详情页面时，无法获取teacher数据
      ...
      initCourseInfo() {
        courseApi.getCourseInfo(this.courseId).then(response => {
          this.course = response.data.data.course,
            this.firstLevelSubject = response.data.data.firstLevelSubject,
            this.secondLevelSubject = response.data.data.secondLevelSubject,
      ...
      //点击立即购买创建订单
      createOrders() {
        orderApi.createOder(this.course.id).then(response => {
